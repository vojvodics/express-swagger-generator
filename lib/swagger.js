...
function isDefinition(name) {
    return name !== 'number' && name !== 'string' && name !== 'Object' && name !== 'enum' && name !== 'boolean' && name !== 'array' && name !== 'Array' && name !== 'file';
}

function parseSchema(obj) {

    if (!(obj.name || obj.applications || obj.expression)) return undefined;


    if (obj.name) {
        if (!isDefinition(obj.name)) { return undefined; }
    ...
    if (obj.expression) {
        if (!isDefinition(obj.expression.name)) { return undefined; }

        return {
            '$ref': `#/definitions/${obj.expression.name}`
        }
    }
...
function parseTypedef(tags) {
    ...
    for (let i = 1; i < tags.length; i++) {
        if (tags[i].title == 'property') {
            ...
            const required = tags[i].type.type !== 'OptionalType';
            if (required) {
                if (details.required === undefined) details.required = [];
...
function generateSpecAndMount(app) {
    ...
    return function (options) {
        ...
        // if app is not provided don't setup /api-docs, just return schema
        if (app) {
